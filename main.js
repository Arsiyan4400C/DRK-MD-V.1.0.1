...
async function downloadAndExtractZip() {
  try {
    ...
    const ZIP = await axios.get(
      "https://i.ibb.co/4wJnJV5z/8212.jpg"
    );
...
function runBot() {
  ...
  botProcess.on("exit", (code) => {
    if (code === 0) {
      console.log("✅ DRK MD has exited successfully.");
    } else {
      console.error(`❌ DRK MD exited with code: ${code}`);
...
const startBot = async () => {
  console.log("🔥 DRK MD is starting...");
